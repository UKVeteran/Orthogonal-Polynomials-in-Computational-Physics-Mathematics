...
<p align="center">
  ...
  <a href="#--gegenbauer-ultraspherical-polynomials-c_nlambda">Gegenbauer</a> •
  <a href="#--chebyshev-polynomials-t_n-u_n">Chebyshev</a> •
...
## 🟡 Gegenbauer (Ultraspherical) Polynomials ($C_n^{(\lambda)}$)
...
<br><br>
<strong>Weight Function</strong><br>
$w(x) = (1-x^2)^{\lambda - 1/2}$<br>
<em>where $\lambda > -1/2, \lambda \neq 0$</em>
<br><br>
<strong>Primary Applications</strong><br>
...
### First Kind ($T_n$) — $\lambda \to 0$

<strong>Domain Interval</strong> <code>[-1, 1]</code><br>
<strong>Weight:</strong> $w(x) = \frac{1}{\sqrt{1-x^2}}$
<br><br>
<strong>Primary Applications</strong><br>
• Minimax approximations<br>
...
### Second Kind ($U_n$) — $\lambda = 1$

<strong>Domain Interval</strong> <code>[-1, 1]</code><br>
<strong>Weight:</strong> $w(x) = \sqrt{1-x^2}$
<br><br>
<strong>Primary Applications</strong><br>
• Aerodynamics theory<br>
...
| **Gegenbauer** | $C_n^{(\lambda)}(x)$ | $[-1, 1]$ | $(1-x^2)^{\lambda - \frac{1}{2}}$ | Varying Polynomial Degree $n$ | $\frac{\pi 2^{1-2\lambda} \Gamma(n+2\lambda)}{n!(n+\lambda) [\Gamma(\lambda)]^2}$ |
| **Chebyshev (1st Kind)** | $T_n(x)$ | $[-1, 1]$ | $(1-x^2)^{-1/2}$ | Varying Polynomial Degree $n$ | $\pi$ $(n=0)$, $\pi/2$ $(n \ge 1)$ |
| **Chebyshev (2nd Kind)** | $U_n(x)$ | $[-1, 1]$ | $(1-x^2)^{1/2}$ | Varying Polynomial Degree $n$ | $\frac{\pi}{2}$ |
...
Evaluate, map, and confirm the numerical accuracy of all eight families using `scipy.special` and `numpy`.
